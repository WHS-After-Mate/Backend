...
- **`reference_guides`의 새 care_type 5종(energy_lifting/botox/filler/skin_booster/hair_removal)은 미검수 문구** — 2026-08-17 `treatment_catalog` 등록을 막던 `INVALID_CARE_TYPE`을 풀려고 스텁으로 추가한 것으로, 실제 의료진 검수를 거치지 않은 일반적인 사후관리 주의사항일 뿐이다. `reference_guides.reviewed_by`/`reviewed_at` 컬럼이 있지만 코드 어디서도 이 값을 검사하지 않아, 검수 여부와 무관하게 `GET /aftercare/daily-guide`·`POST /aftercare/questions`의 LLM이 이 5종을 기존 `peeling`/`laser_toning`과 똑같이 "검수된 사실 근거"로 취급해 실제 고객에게 노출한다(`docs/llm-prompt-design.md`가 명시한 "검수된 가이드만 근거로 삼는다" 원칙과 어긋남). 프로덕션 전 반드시 의료진 검수 후 문구 교체 필요 — 위 위험 신호 키워드 항목과 동일한 성격의 미검수 리스크.
